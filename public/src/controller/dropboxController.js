class dropBoxController {
    ...
    connectFirebase = () => {
        var firebaseConfig = {
            apiKey: "AIzaSyC6Txo_uDPBD11CKwgrOOfMxy0MmqCUllM",
            authDomain: "dropbox-clone-22e22.firebaseapp.com",
            databaseURL: "https://dropbox-clone-22e22.firebaseio.com",
            projectId: "dropbox-clone-22e22",
            storageBucket: "dropbox-clone-22e22.appspot.com",
            messagingSenderId: "717864095656",
            appId: "1:717864095656:web:6d1fff23bc659a93"
        };
        ...
        this.inputFilesEl.addEventListener('change', event => {
            ...
            this.uploadTask(event.target.files).then(responses => {
                responses.forEach(resp => {
                    let pathReference = storage.ref(resp.fullPath);
                    pathReference.getDownloadURL().then(response => {
                        let url = response
    ...
    ajax = (method = 'GET', url, formData = new FormData(), onprogress = () => { }, onloadstart = () => { }) => {
        return new Promise((resolve, reject) => {
    ...
    initEventsLi = li => {
        ...
        li.addEventListener('click', event => {
            if (event.shiftKey) {
                ...
                if (firstLi) {
                    ...
                    lis.forEach((item, index) => {
                        if (firstLi === item) indexStart = index
                        if (li === item) indexEnd = index
                    })
            ...
            if (!event.ctrlKey) {
                this.listFilesEl.querySelectorAll('li.selected').forEach(item => {
